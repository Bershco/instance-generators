# Task: Implement mprime instance generator
...
git checkout main
git pull --ff-only
git checkout -b feature/mprime-generator
```
...
- `problem_generator/mprime/template.pddl`
- `problem_generator/mprime/generator.py`
...
- Search recursively under `problems/numeric/mprime/`
...
locations, keys/locks or other access-control objects if present, fuel/resource fluents, connectivity structure, and goals
...
```bash
git add problem_generator/mprime/template.pddl
git commit -m "Add template.pddl for mprime"
git push -u origin feature/mprime-generator
```
...
```bash
git add problem_generator/mprime/generator.py
git commit -m "Add generator.py for mprime"
git push
```
...
```bash
gh pr create --title "Add mprime generator" --body "Adds template and generator implementation for mprime"
```
