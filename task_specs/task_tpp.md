# Task: Implement tpp instance generator
...
git checkout main
git pull --ff-only
git checkout -b feature/tpp-generator
```
...
- `problem_generator/tpp/template.pddl`
- `problem_generator/tpp/generator.py`
...
- Search recursively under `problems/numeric/tpp/`
...
markets/depots/locations, products, vehicles if present, cost/capacity fluents, purchase/transport structure, and goals
...
```bash
git add problem_generator/tpp/template.pddl
git commit -m "Add template.pddl for tpp"
git push -u origin feature/tpp-generator
```
...
```bash
git add problem_generator/tpp/generator.py
git commit -m "Add generator.py for tpp"
git push
```
...
```bash
gh pr create --title "Add tpp generator" --body "Adds template and generator implementation for tpp"
```
