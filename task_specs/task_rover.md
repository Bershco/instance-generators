# Task: Implement rover instance generator
...
git checkout main
git pull --ff-only
git checkout -b feature/rover-generator
```
...
- `problem_generator/rover/template.pddl`
- `problem_generator/rover/generator.py`
...
- Search recursively under `problems/numeric/rover/`
...
rovers, waypoints, objectives or samples if present, stores/cameras if present, numeric resource fluents, movement/visibility structure, and goals
...
```bash
git add problem_generator/rover/template.pddl
git commit -m "Add template.pddl for rover"
git push -u origin feature/rover-generator
```
...
```bash
git add problem_generator/rover/generator.py
git commit -m "Add generator.py for rover"
git push
```
...
```bash
gh pr create --title "Add rover generator" --body "Adds template and generator implementation for rover"
```
