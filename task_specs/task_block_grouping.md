# Task: Implement block-grouping instance generator
...
`problems/numeric/block-grouping/`
...
- Use a dedicated branch for this domain:
  - `feature/block-grouping-generator`
...
git checkout main
git pull --ff-only
git checkout -b feature/block-grouping-generator
```
...
- `problem_generator/block-grouping/template.pddl`
- `problem_generator/block-grouping/generator.py`
...
- Search recursively under `problems/numeric/block-grouping/`
...
blocks, groups, numeric values, grouping constraints, target equations/values, and naming conventions
...
```bash
git add problem_generator/block-grouping/template.pddl
git commit -m "Add template.pddl for block-grouping"
git push -u origin feature/block-grouping-generator
```
...
```bash
git add problem_generator/block-grouping/generator.py
git commit -m "Add generator.py for block-grouping"
git push
```
...
```bash
gh pr create --title "Add block-grouping generator" --body "Adds template and generator implementation for block-grouping"
```
